...
void * producer_thread( void *arg)
{
    ...
    struct list *my_local = malloc(sizeof(struct list));
    my_local->header = my_local->tail = NULL;
    ...
    while( counter  < K )
    {
        ...
        if( my_local->header == NULL )
        {
            my_local->header = my_local->tail = ptr;
        }
        else
        {
            my_local->tail->next = ptr;
            my_local->tail = ptr;
        }                    
        ++counter;
    }
    ...
    if( List->header == NULL )
    {
        List->header = my_local ->header;
        List->tail = my_local -> tail;
    }
    else
    {
        List->tail->next = my_local -> header;
        List->tail = my_local -> tail;
    }
